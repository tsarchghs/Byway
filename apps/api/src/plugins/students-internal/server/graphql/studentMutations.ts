...
export const StudentMutation = extendType({
  ...
  definition(t) {
    ...
    // ✅ Added surgically (moved inside): enroll a student into a classroom
    ...
    // ✅ Also moved inside: bulk enrollment via CSV
    t.field("bulkEnrollCsv", {
      ...
      async resolve(_root, args, ctx) {
        ...
        for (const line of lines) {
          const [studentId, courseId, classroomId] = line
            .split(",")
            .map((s) => s?.trim())
