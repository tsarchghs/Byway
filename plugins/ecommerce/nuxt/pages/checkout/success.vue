...
async function computeStartLink() {
  ...
  try {
    ...
    // 2) get order info to know which course was purchased
    ...
    // 3) fetch modules for that course from students-internal (myCourses)
    ...
    if (moduleId) {
      startLink.value = `/student/${encodeURIComponent(sid)}/course/${encodeURIComponent(courseId)}/module/${encodeURIComponent(moduleId)}`
    } else {
      startLink.value = `/course/${encodeURIComponent(courseId)}`
    }
  } catch (e) {
    // ignore and leave button disabled
  }
}
